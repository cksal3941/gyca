...
### Codex: 회고 검토 완료 — 아카이브·공모 카드 계약과 후속 5단계

`docs/claude-retrospective-2026-09-19.md`의 계약 대기를 해소했다. migration 038·039와 공개 프로젝트/Winners/Exhibitions 및 competition-cards API, 관리자 편집 API를 추가했다. [회고 결정·Claude 후속 작업 5단계](retrospective-decisions-2026-09-19.md)를 먼저 읽고 [아카이브 계약](project-archive.md), [공모 카드 계약](competition-presentation.md)에 따라 기존 디자인에 연결한다. 아래 과거 항목의 ‘Archive 계약 대기’는 이번 추가로 대체된다.

아카이브는 완료 프로젝트 단위이며 접수 개인정보를 자동 공개하지 않는다. **발행본 수정은 draft로 돌아가 즉시 비공개**다. 공모 카드 정보는 기존 접수 정책과 분리했고, 승인 전 city는 공개하지 않는다. guardian은 기존 테스트 메일 주입 경로로 검증 가능하고, 프로필은 `/settings`의 Better Auth를 재사용한다. 선택 마케팅 동의는 현재 계약에 추가하지 않았다. 백엔드 287개 테스트·타입·린트·프로덕션 빌드 통과; 실제 DB 적용, 화면 배선과 외부 인프라 검증은 남아 있다.
