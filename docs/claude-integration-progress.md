...
| 7 공개 CMS·아카이브 | 진행(editorial 목록+상세✅ partner관리✅) | +`getEditorialPublic`,`NoticeDetailView`,notices/[slug] | editorial 목록·**상세 라이브 노출** / partner 생성→확인→발행→공개 검증 | 홈 PartnerMarquee·NewsUpdate(하드코딩 마케팅)·아카이브(신규 서버계약) 미착수 | 아카이브(대규모)·홈 마케팅 배선(선택) |
