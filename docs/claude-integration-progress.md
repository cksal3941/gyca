...
| 5 운영·보조 기능 | 진행(조회✅ **개인정보요청✅**) | `ops.ts`(dashboard/health/reviews),`admin/payments`,`index.ts`(privacy 어댑터),mypage privacy 패널 | 대시보드 실데이터·게이트·gross 주석 / **개인정보 요청 생성→접수됨→취소(서버 cancelled) 실검증** | accept-late/requeue/환불 mutations·인증서·프로필 편집(계약없음) 미착수 | 결제 mutations·보조 기능 |
