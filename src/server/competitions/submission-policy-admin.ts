import { z } from "zod";
import { SubmissionPolicyDetailSchema, UpdateSubmissionPolicySchema } from "../../contracts/submission-policy.ts";
...
const rowSchema = z.object({ revision: z.number().int(), submission_policy: z.unknown(), draft_enabled: z.boolean(), payment_enabled: z.boolean() });
...
export function createSubmissionPolicyHandlers(deps: {
  ...
  return {
    get: (request: Request, competitionId: string) => run(request, false, 200, actor => deps.database.transaction(async db => {
      const id = parseRequest(identifier, competitionId); await authorize(db, actor);
      ...
      return SubmissionPolicyDetailSchema.parse({ competitionId: id, revision: row.revision, policy: row.submission_policy });
    })),
    update: (request: Request, competitionId: string) => run(request, true, 200, async actor => {
      ...
      const input = parseRequest(UpdateSubmissionPolicySchema, await jsonBody(request));
      return deps.database.transaction(async db => {
        ...
        await db.query("UPDATE gyca_competitions SET submission_policy=$2::jsonb,revision=$3 WHERE id=$1", [id, JSON.stringify(input.policy), revision]);
        await db.query("INSERT INTO gyca_submission_policy_changes VALUES($1,$2,$3,$4::jsonb,$5)", [id, revision, actor, JSON.stringify(input.policy), deps.now()]);
        return SubmissionPolicyDetailSchema.parse({ competitionId: id, revision, policy: input.policy });
      });
    }),
  };
}
